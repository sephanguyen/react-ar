...
!(function(global) {
  'use strict';
  var DeviceOrientationController = function(object, domElement, options) {
    (this.object = object),
      (this.element = domElement || document),
      (this.freeze = !0),
      (this.enableManualDrag = !0),
      (this.enableManualZoom = !0),
      (this.useQuaternions = !0),
    ...
    var scrollSpeedX,
      ...
      CONTROLLER_STATE = { AUTO: 0, MANUAL_ROTATE: 1, MANUAL_ZOOM: 2 },
      ...
      startFOVFrustrumHeight =
        2e3 * Math.tan(THREE.Math.degToRad((this.object.fov || 75) / 2)),
    ...
    (this.constrainObjectFOV = function() {
      (relativeFOVFrustrumHeight =
        ...
        (relativeVerticalFOV = THREE.Math.radToDeg(
          2 * Math.atan(relativeFOVFrustrumHeight / 2e3)
        )),
      ...
      (this.onCompassNeedsCalibration = function(event) {
        event.preventDefault(), fireEvent(CONTROLLER_EVENT.CALIBRATE_COMPASS);
      }.bind(this)),
      ...
      (this.onDocumentMouseMove = function(event) {
        (currentX = event.pageX), (currentY = event.pageY);
      }.bind(this)),
    ...
    var createQuaternion = (function() {
        var finalQuaternion = new THREE.Quaternion(),
          ...
          worldTransform = new THREE.Quaternion(
            -Math.sqrt(0.5),
            0,
            0,
            Math.sqrt(0.5)
      ...
      createRotationMatrix = (function() {
        var finalMatrix = new THREE.Matrix4(),
          deviceEuler = new THREE.Euler(),
          screenEuler = new THREE.Euler(),
          worldEuler = new THREE.Euler(-Math.PI / 2, 0, 0, 'YXZ'),
          screenTransform = new THREE.Matrix4(),
          worldTransform = new THREE.Matrix4();
        return (
          worldTransform.makeRotationFromEuler(worldEuler),
          function(alpha, beta, gamma, screenOrientation) {
            return (
              deviceEuler.set(beta, alpha, -gamma, 'YXZ'),
    ...
    (this.updateManualMove = (function() {
        ...
        objQuat.copy(tmpQuat),
          appState === CONTROLLER_STATE.MANUAL_ROTATE
            ? ((lat = (startY - currentY) * scrollSpeedY),
              ...
              options.naturalScrolling &&
                ((lat = 1 === Math.sign(lat) ? -Math.abs(lat) : Math.abs(lat)),
                (lon = 1 === Math.sign(lon) ? -Math.abs(lon) : Math.abs(lon))),
              ...
              (realZ = rotation.setFromQuaternion(deviceQuat || tmpQuat, 'YXZ')
                .z),
              rotQuat.set(
                0,
                0,
                Math.sin((realZ - tmpZ) / 2),
                Math.cos((realZ - tmpZ) / 2)
              ),
              tmpQuat.multiply(rotQuat),
              rotQuat.set(
                0,
                0,
                Math.sin((realZ - objZ) / 2),
                Math.cos((realZ - objZ) / 2)
              ),
              objQuat.multiply(rotQuat),
      ...
      (this.updateDeviceMove = (function() {
        var alpha, beta, gamma, orient, deviceMatrix;
        return function() {
          if (
            ((alpha = THREE.Math.degToRad(this.deviceOrientation.alpha || 0)),
            (beta = THREE.Math.degToRad(this.deviceOrientation.beta || 0)),
            (gamma = THREE.Math.degToRad(this.deviceOrientation.gamma || 0)),
            (orient = THREE.Math.degToRad(this.screenOrientation || 0)),
            0 !== alpha && 0 !== beta && 0 !== gamma)
          ) {
            if (
              (this.useQuaternions
                ? (deviceQuat = createQuaternion(alpha, beta, gamma, orient))
                : ((deviceMatrix = createRotationMatrix(
                    alpha,
                    beta,
                    gamma,
                    orient
      ...
      (this.update = function() {
        this.updateDeviceMove(),
          appState !== CONTROLLER_STATE.AUTO && this.updateManualMove();
      }),
      (this.connect = function() {
        window.addEventListener('resize', this.constrainObjectFOV, !1),
      ...
      (this.disconnect = function() {
        (this.freeze = !0),
          window.removeEventListener('resize', this.constrainObjectFOV, !1),
  ...
  (DeviceOrientationController.prototype = Object.create(
    ...
    'function' == typeof define && define.amd
      ...
      ? ('undefined' != typeof module &&
          module.exports &&
          (exports = module.exports = DeviceOrientationController),
        (exports.DeviceOrientationController = DeviceOrientationController))
      : (global.DeviceOrientationController = DeviceOrientationController);
})(this);
